...
const BOAT_CLEARANCE = 0.1; // vertical distance from water crest to boat bottom

function BoatModel({ scale = 0.02 }: { scale?: number }) {
  const { scene } = useGLTF("/models/boat_optimized.glb", true); // Use compressed version
  ...
  return (
    <group scale={scale} rotation={[0, Math.PI / 2, 0]} position={[0, 0, 0]}>
...
// Preload GLB model
useGLTF.preload("/models/boat_optimized.glb");